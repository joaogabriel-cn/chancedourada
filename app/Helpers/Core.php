...
class Core
{
    ...
    public static function generateGameHistory($userId, $type, $win, $bet, $changeBonus, $tx)
    {
        ...
        Log::info('Gerando historico de jogo', [
            'user_id' => $userId,
            'type' => $type,
            'win' => $win,
            'bet' => $bet,
        ...
        if($type == 'bet' && !empty($user->inviter)) {
            ...
            $transaction = Order::where('transaction_id', $tx)->where('type', 'Raspadinha')->where('status', 1)->first();
    ...
    private static function PayWin($user, $affiliate, $type, $amount, $changeBonus)
    {
        ...
        Log::info('PayWin', [
            ...
            'amount' => $amount,
            'changeBonus' => $changeBonus,
            '$setting->revshare_reverse' => $setting->revshare_reverse
        ]);

        /// subtrair afiliado ganhos
        if($setting->revshare_reverse) {
            Log::info('PayWin Reverse', [
                'user_id' => $user->id,
                'affiliate_id' => $affiliate->id,
                'type' => $type,
                'amount' => $amount,
            ...
            if($type == 'win' && !empty($user->inviter) && in_array($changeBonus, ['balance', 'balance_withdrawal'])) {
                if(!empty($affiliate)) {
                    ...
                    if(!empty($affHistoryRevshare)) {
                        Log::info('Affiliate History Revshare', [
                            'user_id' => $user->id,
                            'affiliate_id' => $affiliate->id,
                            'commission_type' => $affHistoryRevshare->commission_type,
                            'status' => $affHistoryRevshare->status
                        ]);
                        /// verificar qual revshare pagar, o fake ou o padrão
