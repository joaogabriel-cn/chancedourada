...
class AuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        try {
            ...
            {
                // Associação de afiliado (inviter) e criação de histórico CPA quando aplicável
                if(isset($request->reference_code) && !empty($request->reference_code)) {
                    ...
                    if(!empty($checkAffiliate)) {
                        ...
                        // cria histórico inicial (CPA ou placeholder) para aparecer no painel
                        try {
                            self::saveAffiliateHistory($user);
                        } catch (\Throwable $e) {
                            \Log::warning('Falha ao salvar histórico de afiliado no registro', [
                                'user_id' => $user->id,
                                'error' => $e->getMessage()
                            ]);
                        }
