...
use App\Filament\Admin\Pages\DefaultPrizesPage;
...
class AdminPanelProvider extends PanelProvider
{
    ...
    public function panel(Panel $panel): Panel
    {
        return $panel
                        ...
                        NavigationGroup::make('Definições')
                            ...
                            ->items([
                                ...
                                NavigationItem::make('setting-mail')
                                    ...
                                    ->visible(fn(): bool => auth()->user()->hasRole('admin')),
                                NavigationItem::make('default-prizes')
                                    ->icon('heroicon-o-gift')
                                    ->label(fn (): string => 'Prêmios Padrão')
                                    ->url(fn (): string => DefaultPrizesPage::getUrl())
                                    ->isActiveWhen(fn () => request()->routeIs('filament.pages.default-prizes-page'))
                                    ->visible(fn(): bool => auth()->user()->hasRole('admin')),
                            ])
                        : NavigationGroup::make()
                    ,
            ...
            /*->renderHook(
                'panels::body.start',
                fn () => view('filament.components.login-background')
            )*/
            ->renderHook(
                'panels::auth.login.form.before',
                fn () => view('filament.components.login-header')
            )
            /*->renderHook(
                'panels::auth.login.form.after',
                fn () => view('filament.components.login-footer')
            )*/
